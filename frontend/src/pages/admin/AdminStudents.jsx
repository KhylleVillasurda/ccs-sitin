  ...
  const save = async () => {
    ...
    try {
      await api.put(`/students/${student.id_number}`, {
        ...
        remaining_sessions: parseInt(form.remaining_sessions),
      })
      onSaved()
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        <div className="modal-header">
          <span className="modal-title">✏️ Edit Student — {student.id_number}</span>
        ...
        <div className="form-row">
          ...
          <div className="form-group">
            ...
            <input className="input" name="remaining_sessions" type="number" min="0" max="30"
              value={form.remaining_sessions} onChange={handleChange} />
          </div>
        </div>
        <div className="form-group">
...
function AddModal({ onClose, onSaved }) {
  const [form, setForm] = useState({
    ...
    course_level: 1, password: 'student123', email: '', course: 'BSIT', address: ''
  })
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')
  ...
  const save = async () => {
    setLoading(true); setError('')
    try {
      await api.post('/students/', { ...form, course_level: parseInt(form.course_level) })
      onSaved()
    } catch (err) { setError(err.response?.data?.error || 'Failed') }
    finally { setLoading(false) }
  }
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <div className="form-group">
          ...
          <input className="input" name="id_number" value={form.id_number} onChange={handleChange} placeholder="2024-00001" />
        </div>
        <div className="form-row">
          <div className="form-group">
            <label className="label">Last Name</label>
        ...
        <div className="form-group">
          ...
          <input className="input" name="password" value={form.password} onChange={handleChange} />
        </div>
        <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end', marginTop: '0.5rem' }}>
          <button className="btn btn-ghost" onClick={onClose}>Cancel</button>
          <button className="btn btn-primary" onClick={save} disabled={loading}>
...
export default function AdminStudents() {
  ...
  const deleteStudent = async (id_number, name) => {
    if (!confirm(`Delete ${name}?`)) return
    await api.delete(`/students/${id_number}`)
    load()
  }
  ...
  const resetAll = async () => {
    ...
    setMsg('All sessions reset!'); load()
    ...
    <div>
      ...
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          ...
          <button className="btn btn-ghost btn-sm" onClick={resetAll}>🔄 Reset Sessions</button>
          <button className="btn btn-primary btn-sm" onClick={() => setAdding(true)}>➕ Add Student</button>
        </div>
      </div>
              ...
              {loading ? (
                ...
                <tr><td colSpan={6}>
                  <div className="empty-state">
                    ...
                    <div className="empty-state-text">{search ? 'No students match your search' : 'No students registered yet'}</div>
                  </div>
                </td></tr>
              ) : filtered.map(s => (
                <tr key={s.id_number}>
                  <td><span style={{ color: 'var(--blue)', fontFamily: 'monospace' }}>{s.id_number}</span></td>
                  <td style={{ fontWeight: 500 }}>{s.last_name}, {s.first_name} {s.middle_name}</td>
                  <td><span className="badge badge-blue">{s.course_level}</span></td>
                  <td>{s.course || 'BSIT'}</td>
                  <td>
                    <span className={`badge ${s.remaining_sessions > 10 ? 'badge-green' : s.remaining_sessions > 0 ? 'badge-orange' : 'badge-red'}`}>
                      {s.remaining_sessions}
                    </span>
                  </td>
                  <td>
                    <div style={{ display: 'flex', gap: '0.4rem' }}>
                      <button className="btn btn-ghost btn-sm" onClick={() => setEditing(s)}>✏️ Edit</button>
                      <button className="btn btn-danger btn-sm" onClick={() => deleteStudent(s.id_number, `${s.first_name} ${s.last_name}`)}>🗑</button>
                    </div>
                  </td>
                </tr>
              ))}
