...
function SitInModal({ onClose, onStarted }) {
  ...
  const findStudent = async () => {
    ...
    try {
      const { data } = await api.get(`/students/${search.trim()}`)
      setStudent(data)
  ...
  const startSitin = async () => {
    ...
    try {
      await api.post('/sitin/start', { student_id: student.id_number, ...form })
      onStarted()
      ...
      <div className="modal">
        ...
        {student && (
          <div style={{ background: 'var(--bg2)', borderRadius: '8px', padding: '1rem', marginBottom: '1rem', border: '1px solid var(--border)' }}>
            <div style={{ fontWeight: 600, marginBottom: '0.25rem' }}>{student.first_name} {student.last_name}</div>
            <div style={{ fontSize: '0.8rem', color: 'var(--fg-dim)' }}>
              {student.id_number} · {student.course} Year {student.course_level}
            </div>
            <div style={{ marginTop: '0.5rem', fontSize: '0.85rem' }}>
              Remaining sessions:{' '}
              <span className={`badge ${student.remaining_sessions > 10 ? 'badge-green' : student.remaining_sessions > 0 ? 'badge-orange' : 'badge-red'}`}>
    ...
    <div>
      ...
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          ...
          <button className="btn btn-primary btn-sm" onClick={() => setShowModal(true)}>➕ New Sit-in</button>
        </div>
      </div>

      <div className="card" style={{ marginBottom: '1rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: 'var(--green)', boxShadow: '0 0 6px var(--green)' }} />
            ...
            {loading ? (
              ...
              <tr><td colSpan={8}>
                <div className="empty-state">
                  ...
                  <div className="empty-state-text">No students currently sitting in</div>
                </div>
              </td></tr>
            ) : current.map(r => (
              <tr key={r.id}>
                <td><span style={{ color: 'var(--fg-dim)', fontFamily: 'monospace' }}>#{r.id}</span></td>
                <td><span style={{ color: 'var(--blue)', fontFamily: 'monospace' }}>{r.student_id}</span></td>
                ...
                <td>
                  <button className="btn btn-danger btn-sm" onClick={() => endSitin(r.id)} disabled={ending === r.id}>
                    {ending === r.id ? '…' : '⏹ End'}
                  </button>
                </td>
              </tr>
