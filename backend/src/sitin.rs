...
pub async fn start_sitin(
    ...
    let student =
        student.ok_or((Status::NotFound, Json(ApiError { error: "Student not found".into() })))?;

    if student.remaining_sessions <= 0 {
        return Err((
            Status::BadRequest,
            Json(ApiError { error: "No remaining sessions".into() }),
        ));
    }

    // Block duplicate active sit-in
    ...
    if active.0 > 0 {
        return Err((
            Status::Conflict,
            Json(ApiError { error: "Student is already sitting in".into() }),
        ));
    }
    ...
    .bind(&req.purpose)
    .bind(&req.lab)
    .bind(student.remaining_sessions)
    .execute(&mut **db)
    .await
    .map_err(|_| (Status::InternalServerError, Json(ApiError { error: "Failed to start sit-in".into() })))?;
...
pub async fn end_sitin(
    ...
    sqlx::query(
        "UPDATE sit_in_records SET status = 'done', time_out = datetime('now') \
...
pub async fn student_records(
    ...
    // Allow both admin and the student themselves
